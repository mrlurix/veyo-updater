...
(function () {
  ...
  fetch("assets/search-index.json?v=1").then(function (r) { return r.json(); }).then(function (j) { index = j; }).catch(function () { index = []; });
  ...
  function render() {
    ...
    if (!items.length) {
      ...
      empty.textContent = index === null ? "Loading index…" : (lastQ ? "No results for “" + lastQ + "”" : "Type to search the docs…");
